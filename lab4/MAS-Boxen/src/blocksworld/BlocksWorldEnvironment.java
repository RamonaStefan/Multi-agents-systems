...
public abstract class BlocksWorldEnvironment implements Environment {
	...
	public static class AgentData {
		...
		public String toString() {
			return "Agent " + agent + " at " + station + " holds: " + (holding == null ? "none" : holding)
					+ "; prev action: " + (previousActionSucceeded ? "OK" : "failed") + "; plan: " + plan + "; "
					+ agent.statusString() + "\n";
		}
	}
	...
	public boolean step() {
		...
		for(AgentData ag : agents) {
			...
			Action result = ag.getAgent().response(
					new BlocksWorldPerceptions(currentStack, agentStation, ag.hasPreviousActionSucceeded(), ag.plan));
			...
			case NEW_PLAN:
				...
				ag.plan.add(0, new BlocksWorldAction(Type.NONE)); // penalize the creation of a new plan.
				break;
			default:
				break;
			}
			
			BlocksWorldAction act = ag.plan.size() > 0 ? ag.plan.remove(0) : new BlocksWorldAction(Type.NONE);
