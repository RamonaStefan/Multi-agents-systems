...
public abstract class BlocksWorldEnvironment implements Environment {
	...
	public String toString() {
		...
		for(AgentData a : getAgents()) {
			...
			// data.add("\n");
			if (stations.indexOf(a.getStation()) != -1) {
	...
	public boolean step() {
		...
		for(AgentData ag : agents) {
			...
			System.out.println("Agent " + ag.agent.toString() + " respondend: " + planningAction);
			...
			case MODIFY_PLAN:
				if(planningAction.getNumberOfActionsToRemove() > 0) {
					...
					int size = ag.plan.size();
					for(int i = 0; i < Math.min(size - 1, n); i++) {
						ag.plan.remove(0);
					}
				}
